...
class ExamDetailAdapter : BaseRecyclerViewAdapter<ExamDetail>(){
    ...
    var doExamItem: (position: Int, trafficsLearn: ExamDetail, result : Int, answerChoose: String ,indexAnswer: Int) -> Unit =
        { _: Int, _: ExamDetail, _: Int, _: String ,_: Int -> }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        model.answer_list?.forEachIndexed { index, result ->
            if (result.isNotEmpty()) {
                list.add(ExamResult(index + 1, result,model.positionChoose == index,result == model.result))
            }
        }
        ...
        adapter.onClickItem = { index, view ->
            if (adapter.getItems(index).answer.equals(model.result)){
                adapter.getItems(index).answer?.let { doExamItem(position, model,1, it,index) }
            } else {
                adapter.getItems(index).answer?.let { doExamItem(position, model,-1, it,index) }
            }
